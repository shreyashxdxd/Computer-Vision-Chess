...
pygame.display.set_caption("Gesture Chess – Stable Control")
...
def reset_selection():
    global state, src, dst, neutral_required
    state = "SRC_FILE"
    src = ""
    dst = ""
...
while running:
    ...
    if result.multi_hand_landmarks:
        for hand_lms, hand_info in zip(result.multi_hand_landmarks,
            ...
            # FILE SELECTION
            if state in ["SRC_FILE", "DST_FILE"] and not neutral_required:
            ...
            # RANK SELECTION
            elif state in ["SRC_RANK", "DST_RANK"] and not neutral_required:
                if fingers in [1,2,3,4]:
                    selecting_hand = label
                    if label == "Right":
    ...
    if neutral_required:
        if (all_fingers_down and
            not current_thumb["Left"] and
            not current_thumb["Right"]):
            neutral_required = False

    # ================= STABLE DETECTION =================

    if detected != stable_value:
        stable_value = detected
        stable_start_time = current_time

    is_stable = (
        stable_value is not None and
        current_time - stable_start_time > STABLE_REQUIRED_TIME
    )
    ...
    for side in ["Left", "Right"]:
        ...
        thumb_edge = current_thumb[side] and not prev_thumb[side]

        if (
            not neutral_required
            and is_stable
            and thumb_edge
    ...
    draw_board()
    draw_pieces()
    draw_text(state, stable_value, src, dst)
